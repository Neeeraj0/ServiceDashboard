...
export default function AssignSiteSurvey({
  preOrderId,
  executiveId,
  parentPreOrderId,
  materialsdetails,
  ...
  const handleAssignTask = async (e: React.FormEvent) => {
    ...
    const transformedMaterials = materialsdetails?.map(material => ({
    ...
    const taskDataCreation = {
      title: "Site Survey Task",
      // customerComplaint: customerComplaint,
      description: "Site Survey task",
    ...
    //   await axios.post(`${process.env.NEXT_PUBLIC_SERVICE_BACKEND_API}/api/tasks`, taskDataCreation, {
      ...
      const siteSurveyData = {
        _id: preOrderId,
        executive_id: executiveId || "N/A",
        acDetails: ac_units,
        materialdetails: { extraMaterials: transformedMaterials },
        timestamp: new Date()
      };
