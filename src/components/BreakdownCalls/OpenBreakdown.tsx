...
const OpenBreakdown: React.FC<OpenBreakdownProps> = ({ onLoadingComplete }) => {
  ...
  const [totalItems, setTotalItems] = useState(0); // <-- total count from server
  ...
  const [isFilterActive, setIsFilterActive] = useState(false); // track if filter mode is on
  ...
  try {
    ...
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_CIRCOLIFE_PRODUCTION_API}/api/query/get/queries/all`,
      {
    ...
    const { allQueries, pagination } = res.data;

    const fetchedData = (allQueries ?? []).map((order: any) => ({
      ...order,
    ...
    setTotalItems(pagination?.totalRecords ?? fetchedData.length); // ✅ correct field
  ...
  // Re-fetch when page changes OR refresh is triggered (but only when NOT in filter mode)
  useEffect(() => {
    if (!isFilterActive) {
  ...
  // ─── FILTER FETCH (still client-side paged via the filter endpoint) ────────
  const fetchFilteredData = async (filters: FilterParams) => {
    try {
      ...
      const hasFilter =
        ...
        filters.issues.length > 0 || filters.locations.length > 0;
  ...
  const getShippingAddress = (orderId: string) => {
    const address = shippingAddresses.find((a) => a._id === orderId);
  ...
  // ─── ACTIVE / OPEN FILTER (client-side on current page data) ──────────────
  ...
  // ─── PAGINATION ────────────────────────────────────────────────────────────
  // When filters are active, paginate client-side since we have all filtered results.
  // When in normal mode, pagination is server-driven.
  const displayOrders = isFilterActive
    ? searchFilteredOrders.slice(
        (currentPage - 1) * ITEMS_PER_PAGE,
        currentPage * ITEMS_PER_PAGE
      )
    : searchFilteredOrders; // already the current page from server

  const paginationTotal = isFilterActive ? searchFilteredOrders.length : totalItems;

  const paginate = (pageNumber: number) => {
    setCurrentPage(pageNumber);
    // If NOT in filter mode, useEffect will trigger fetchTasks(pageNumber)
  };
  ...
  const handleTaskAssigned = (id: string) => {
    setRemovingId(id);
    setTimeout(() => {
      setFilteredData((prev) => prev.filter((t) => t._id !== id));
      setBackendData((prev) => prev.filter((t) => t._id !== id));
  ...
  const handleMarkAsResolved = (id: string) => {
    ...
    setTimeout(() => {
      const update = (prev: Order[]) =>
        prev.map((t) => t._id === id ? { ...t, status: false } : t)
  ...
  const handleRefresh = () => {
    ...
    setIsFilterActive(false);
    setCurrentFilters({ startDate: null, endDate: null, issues: [], locations: [] });
    setCurrentPage(1);
    triggerRefresh();
  };
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const exportData = searchFilteredOrders.map((order, index) => ({
        "S.No": index + 1,
      ...
      <div className="top-0 bg-white z-20 flex items-center justify-between mb-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 ml-auto">
          <div className="relative group gap-2 lg:flex lg:gap-3 w-full sm:w-auto mb-2 sm:mb-0">
            <button
              className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-[#A14996] border border-[#A14996] rounded-lg hover:bg-[#f9f0f9] w-full sm:w-auto mt-2 lg:mt-0"
              onClick={handleRaiseQueryClassic}
            >
              Raise A Query
            </button>
          </div>
              ...
              displayOrders.map((order) => {
                ...
                return (
                  <tr key={order._id} className={removingId === order._id ? "fade-out" : ""}>
                    ...
                    <td className="p-2 border-b border-blue-gray-50 text-sm max-w-50">{order.subject}</td>
                    <td className="p-2 border-b border-blue-gray-50 text-sm max-w-50">
