...
export default function DatePicker2({
  ...
  const handleDayClick = (selectedDay: string): void => {
    console.log("Selected Day:", selectedDay);
    const day = new Date(selectedDay);
    console.log("Day:", day);
    const formattedDate = new Date(Date.UTC(day.getFullYear(), day.getMonth(), day.getDate())).toISOString().split("T")[0]; 
  ...
  const updateInput = (): string => {
    if (selectedStartDate && selectedEndDate) {
      return `From: ${selectedStartDate} - To: ${selectedEndDate}`;
    } else if (selectedStartDate) {
      return `From: ${selectedStartDate}`;
    } else {
      return "Pick a date";
    }
  };
      ...
      <div className="container">
        <div className="mx-auto max-w-[380px]" ref={datepickerRef}>
          ...
          <input
              ...
              className="h-12 w-full appearance-none rounded-lg border border-stroke bg-white pl-12 pr-4 text-dark outline-none focus:border-primary dark:border-dark-3 dark:bg-dark-2 cursor-pointer "
              value={updateInput()}
          ...
          {isOpen && (
            <div className="absolute shadow-xs flex w-[fit-content] flex-col rounded-lg border border-stroke bg-white px-4 py-6 sm:px-6 sm:py-[30px] dark:border-dark-3 dark:bg-dark-2 cursor-pointer">
              ...
              <div className="flex items-center justify-center space-x-3 pt-4 sm:space-x-4">
                <button className="h-[37px] rounded border border-stroke bg-transparent px-5 text-sm font-medium text-body-color hover:border-primary focus:border-primary dark:border-dark-3 dark:text-dark-6 cursor-pointer">
                  {selectedStartDate}
                </button>
                <button className="h-[37px] rounded border border-stroke bg-transparent px-5 text-sm font-medium text-body-color hover:border-primary focus:border-primary dark:border-dark-3 dark:text-dark-6 cursor-pointer">
                  {selectedEndDate}
