...
import SiteSurveyApprove from '../Dialogs/SiteSurveyApprove';
...
const ApprovalPendingSiteSurvey: React.FC = () => {
  ...
  const itemsPerPage = 10;
    ...
    const handleViewReport = (url: string) => {
      ...
      const handleTaskApproval = (taskId: string) => {
        ...
        setTimeout(() => {
          setBackendData(prevData => prevData.filter(task => task.task_id !== taskId));
          setRemovingIds(prev => {
  ...
  useEffect(() => {
    const fetchCompletedOrders = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_SERVICE_BACKEND_API}/api/siteSurveyDetails/getApprovalPendingSiteSurveys`);
        // const res = await axios.get('http://35.154.208.29:8080/api/siteSurveyDetails/getApprovalPendingSiteSurveys');
            ...
            <tr key={order._id} className="hover:bg-gray-50">
              ...
              <td className="p-2 border-b border-blue-gray-50 text-sm">
                {order.closureDate ? (
                    <>
                    <div>
                    {order.date ? `${formatDate(order.closureDate).time}` : "N/A"}
                    </div>
              ...
              {order.reportPresignedUrl ? (
                ...
                <td className="p-2 border-b border-blue-gray-50 text-sm whitespace-normal break-words max-w-xs z-99999">
                    <SiteSurveyApprove 
                                          orderId={order.task_id} 
                                          taskDetails={order}
                                          onApprove={() => handleTaskApproval(order.task_id)} 
                                        />
                    {/* {hasAssignAccess && (
...
export default ApprovalPendingSiteSurvey;
