...
interface AcUninstallationProps {
  devices: Device[];
}

const AcUninstallationDetails: React.FC<AcUninstallationProps> = ({ devices = [] }) => {
    console.log('Devices:', devices);
  // Calculate counts for different statuses
  const pendingCount = devices.filter(device => 
    device.status === 'Uninstallation Pending').length;
  const requestedCount = devices.filter(device => 
    device.status === 'Uninstallation Open').length;
  const completedCount = devices.filter(device => 
    device.status === 'Uninstallation Completed').length;
                    ...
                    {displayedModels.map(([model, deviceList]) => (
                      <div key={model} className="text-xs">
                        ...
                        <div className="ml-2 space-y-1">
                          {['Uninstallation Pending', 'Uninstallation Open', 'Uninstallation Completed'].map(status => {
                            const count = deviceList.filter(d => d.status === status).length;
                            if (count === 0) return null;
                            
                            const bgColor = 
                              status === 'Uninstallation Pending' ? 'bg-yellow-100 text-yellow-800' :
                              status === 'Uninstallation Open' ? 'bg-blue-100 text-blue-800' :
...
export default AcUninstallationDetails;